...
//
//   用法: node pact-book.mjs [PACT.md] [--out=.pact/pact-book] [--build|--check] [--quiet]
//
// 设计前提（不可动摇）：
//   PACT.md 是**唯一手写真源**，本知识库**全部由它生成**。
//   任何人不得手改 .pact/pact-book/ 下的文件——改了会被 --check 抓出来，并在下次 --build 覆盖。
...
// ── 参数 ────────────────────────────────────────────────────────────────────
let FILE = 'PACT.md', OUT = '.pact/pact-book', MODE = 'build', QUIET = 0
...
const GEN_HEAD = (extra = '') =>
  `> **本页由 \`pact-book.sh\` 从 \`${FILE}\` 自动生成，请勿手改。**\n` +
  `> 改内容请改 \`${FILE}\`，然后重新 \`pact-book.sh --build\`；\`--check\` 会抓出手改导致的漂移。${extra}`
...
for (const ch of chapters) {
  ...
  body = linkify(body, '..')
  put(`src/${ch.path}`,
    `# ${ch.title}\n\n${GEN_HEAD()}\n> 锚点 \`<!-- PACT:${ch.id} -->\`｜真源位置：\`${FILE}\` 第 ${ch.from}–${ch.to} 行\n\n${nav ? nav + '\n\n---\n\n' : ''}${body}\n`)
}
...
for (const r of [...reqs.values()].sort((a, b) => a.id.localeCompare(b.id))) {
  ...
  L.push(`> 相关决策与契约位置聚合在一处，施工时读这一页即可，不必通读 ${FILE} 全文。`)
  L.push(`> **但凡本页与 \`${FILE}\` 有出入，一律以 \`${FILE}\` 为准。**`)
  L.push('')
  L.push('| 项 | 值 |')
  L.push('|---|---|')
  L.push(`| 类型 | ${r.type || '—'} |`)
...
for (const ms of milestones) {
  ...
  L.push(`> S10 施工时按此包切工作单元，队列维护在 \`.pact/board.md\`。`)
  L.push('')
...
// —— 首页 + SUMMARY + book.toml ——
{
  ...
  put('src/index.md', [
    ...
    '',
    '> **规矩**：本知识库任何一页与 `' + FILE + '` 冲突时，**一律以 `' + FILE + '` 为准**。',
...
// ── ⑧ 单文件 HTML 知识库（自研渲染层，零外部二进制）────────────────────────
{
  ...
  const metaLine = LINES.slice(0, 40).filter(l => /创建日期|更新日期|状态[:：]/.test(l))
    ...
    .filter(x => x && x !== '—').join(' · ').replace(/\s{2,}/g, ' ').slice(0, 90) || FILE
  ...
  put('pact-book.html', renderHTML({
    ...
    milestones: milestones.map(m => ({ id: m.id, name: m.name })), navGroups, file: FILE,
  }))
}
